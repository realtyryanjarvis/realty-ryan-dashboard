#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# generate-listing-presentation.sh
# Usage: ./generate-listing-presentation.sh presentations/my-listing/data.json
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail

REPO_DIR="$(cd "$(dirname "$0")" && pwd)"
TEMPLATE="$REPO_DIR/_template/listing-presentation-template.html"
DATA_FILE="${1:-}"

if [[ -z "$DATA_FILE" ]]; then
  echo "❌  Usage: $0 presentations/<slug>/data.json"
  exit 1
fi
...
OUT_FILE="$OUT_DIR/listing-presentation.html"
mkdir -p "$OUT_DIR"

echo "🏗   Building presentation for slug: $SLUG"
...
cd "$REPO_DIR"
git add -A
git commit -m "Add listing presentation: $ADDRESS ($SLUG)"
git push

echo ""
echo "🌐  Live URL:"
echo "    https://homes.realtyryan.com/presentations/$SLUG/listing-presentation.html"
